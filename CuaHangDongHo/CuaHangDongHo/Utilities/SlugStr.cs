...
{
    public class SlugStr
    {
        public string CreateSlug(string title, bool remapToAscii = false, int maxlength = 80)
        {
            if (title == null)
            {
                return string.Empty;
        ...
        private static string RemapInternationalCharToAscii(char character)
        {
            ...
            if ("àåáâäãåąāảạăắằẳẵặâấầẩẫậ".Contains(s))
            {
                return "a";
            }
            ...
            else if ("òóôõöøőỏõọốồổỗộơớờởỡợ".Contains(s))
            {
                return "o";
            }
